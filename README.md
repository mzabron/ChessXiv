...
* Engine analysis - Stockfish 18 runs directly in your browser, so analysis costs the server nothing and works for everyone at once. Switch it on under the board to see between one and five candidate lines (three by default), and set threads, memory and the engine's remaining options from the same panel.
